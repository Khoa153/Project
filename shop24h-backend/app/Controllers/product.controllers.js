...
const getAllProduct = (req, res) => {
    ...
    productModel.find()
        .then((data) => {
            const cateIphone = data.filter(product => product.description === 'Compact')
            const cateMacbook = data.filter(product => product.description === 'Macbook')
            const cateAirpod = data.filter(product => product.description === 'Airpod')
            const cateMacmini = data.filter(product => product.description === 'Macmini')
            const cateDisplay = data.filter(product => product.description === ' Display')
            return res.status(200).json({
                status: 'Get All Product Successfully',
                data,
                cateIphone,
